...
export const getAllTestsApi = async () => {
  try {
    const response = await api.get<Test[]>("/api/Test/GetAll");
...
export const getTestByIdApi = async (testId: number) => {
  try {
    const response = await api.get<Test>(`/api/Test/${testId}`);
    return response.data;
  } catch (err) {
...
export const finishTestApi = async (appId: number) => {
  try {
    const response = await api.put(`/api/Test/FinishTest/${appId}`);
  } catch (err) {
...
export const startTestApi = async (appId: number) => {
  try {
    const response = await api.put(`/api/Test/StartTest/${appId}`);
  } catch (err) {
