...
export const AddApplicationApi = async (aboutYourself: string, expectedMonthlySalary: string, similarExperience: string, cv: File, jobOfferId: number) => {
  try {
    ...
    const response = await api.post("/api/application", formData);
...
export const getMyApplications = async () => {
  try {
    const response = await api.get<applicationModel[]>("/api/application");
    return response.data;
  } catch (err) {
...
export const getCv = async (id: number, fileNameFromApp: string) => {
  try {
    const response = await api.get("/api/application/DownloadCV", {
      params: { id },
      responseType: "blob"
    });
...
const grappapi = "/api/application/GroupedApps";
...
export const getGroupedApplications = async (query?: ApplicationQuery) => {
  ...
  try {
    const response = await api.get<GroupedApplications[]>(url);
    return response.data;
  } catch (err) {
...
export const createTestApi = async (tittle: string, description: string, taskList: number[]) => {
  try {
    const response = await api.post("/api/Test/CreateTest", {
      tittle,
      description,
      taskIds: taskList
    });
...
export const assignTestToAppApi = async (appId: number, testId: number) => {
  try {
    const response = await api.put("/api/application/AssignTestToApp",
      null,
      {
        params: {
          appId,
...
export const rejectApp = async (appId: number) => {
  try {
    const response = await api.put("/api/application/RejectApp", {
      Id: appId
    });
    return response.data;
  } catch (err) {
...
export const getAppEvaluationApi = async (appId: number) => {
  try {
    const response = await api.get<ApplicationEvaluation>(`/api/application/GetAppEvaluation?appId=${appId}`);
    return response.data;
  } catch (err) {
...
export const addAppEvaluationApi = async (data: ApplicationEvaluation) => {
  try {
    const response = await api.post("/api/application/AddAppEvaluation", data);
    return response.data;
  } catch (err) {
...
export const getClassificationApi = async () => {
  try {
    const response = await api.get<GroupedClassification[]>("/api/application/GetClassification");
    return response.data;
  } catch (err) {
...
export const getApplicationById = async (appId: number) => {
  try {
    const response = await api.get<Applications>(`/api/application/${appId}`);
    return response.data;
  } catch (err) {
...
export const inviteToInterview = async (appId: number) => {
  try {
    const response = await api.put(`/api/application/InviteToInterview?appId=${appId}`);
    return response.data;
  } catch (err) {
...
export const GetSummaryForUser = async (userId: string) => {
  try {
    const response = await api.get(`/api/application/UserSummary/${userId}`);
    return response.data;
  } catch (err) {
